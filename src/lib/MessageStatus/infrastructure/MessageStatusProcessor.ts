...
import { MessageStatusUpdateJob, BatchStatusUpdateJob } from './MessageStatusQueue';
...
export class MessageStatusProcessor extends WorkerHost {
  ...
  async process(job: Job<MessageStatusUpdateJob | BatchStatusUpdateJob>): Promise<void> {
    try {
      if (job.name === 'update-status') {
        await this.processSingleStatusUpdate(job as Job<MessageStatusUpdateJob>);
  ...
  private async processSingleStatusUpdate(job: Job<MessageStatusUpdateJob>): Promise<void> {
  ...
  private async processBatchStatusUpdate(job: Job<BatchStatusUpdateJob>): Promise<void> {
    const { updates, batchId } = job.data;
    this.logger.log(`Processing batch ${batchId} with ${updates.length} status updates`);
    ...
    if (failures.length > updates.length * 0.5) {
      throw new Error(`Batch ${batchId} failed: too many individual failures (${failures.length}/${updates.length})`);
    }
  }
}