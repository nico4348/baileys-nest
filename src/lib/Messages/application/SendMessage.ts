...
export class SendMessage {
  ...
  private validateRequest(request: SendMessageRequest): void {
    switch (request.messageType) {
      ...
      case MessageType.MEDIA:
        if (!request.mediaData || !request.mediaData.url || !request.mediaData.mediaType) {
          throw new Error('Media data with url and mediaType is required for media messages');
        }
        break;
      
      case MessageType.REACTION:
        if (!request.reactionData || !request.reactionData.emoji || !request.reactionData.targetMessageId) {
          throw new Error('Reaction data with emoji and targetMessageId is required for reaction messages');
        }
        break;
    }
  }

  private async sendTextMessage(request: SendMessageRequest): Promise<SendMessageResponse> {
  ...
  private async sendMediaMessage(request: SendMessageRequest): Promise<SendMessageResponse> {
  ...
  private async sendReactionMessage(request: SendMessageRequest): Promise<SendMessageResponse> {
    const messageKey = request.reactionData!.messageKey || {
      remoteJid: `${request.to}@s.whatsapp.net`,
      fromMe: false,
      id: request.reactionData!.targetMessageId,
    ...
    return SendMessageResponse.success(messageId, MessageType.REACTION, messageId);
  }
}