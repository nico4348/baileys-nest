...
export class SessionsCreate {
  ...
  async run(
    ...
    rateLimit: number = SessionRateLimit.default().getValue(),
    rateLimitWindow: number = SessionRateLimitWindow.default().getValue(),
  ): Promise<void> {
    const session = new Session(
      ...
      new SessionRateLimit(rateLimit),
      new SessionRateLimitWindow(rateLimitWindow),
    );
